...
function getFriendlyAuthError(err: any): string {
  ...
  // Strip technical "auth/" prefix from unknown errors
    ...
    <div class="min-h-screen flex items-center justify-center px-3 sm:px-4 py-6 sm:py-8 md:py-12 bg-gradient-to-br from-gray-50 via-white to-gray-100">
      ...
      <div class="glass w-full max-w-md p-4 sm:p-6 md:p-8 rounded-2xl border border-black/10 animate-fade-in relative z-10 shadow-xl shadow-black/5">
        ...
        <div class="text-center mb-4 sm:mb-6 md:mb-8">
            ...
            <p class="text-gray-500 mt-1 sm:mt-2 text-xs sm:text-sm md:text-base">Login to continue your prep</p>
        </div>
        ...
        <form (submit)="onSubmit($event)" class="space-y-4 sm:space-y-5">
          <div>
          ...
          <!-- FIX (9): Show/Hide password toggle -->
          <div>
            <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-1 sm:mb-2">Password</label>
            <div class="relative">
              <input [type]="showPassword() ? 'text' : 'password'" [(ngModel)]="password" name="password" id="login-password" required 
          ...
          <button type="submit" [disabled]="isLoading()" id="login-submit"
                  class="w-full bg-black hover:bg-gray-800 text-white font-bold py-2.5 sm:py-3 px-4 rounded-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px] hover:shadow-lg hover:shadow-black/20">
            @if(isLoading()) {
              <span class="flex items-center justify-center gap-2"><i class="fas fa-circle-notch fa-spin"></i> Signing in...</span>
            } @else {
              Sign In
            }
          </button>
        ...
        <button type="button" (click)="onGoogleLogin()" [disabled]="isLoading()" id="login-google"
          ...
          <!-- Local SVG instead of external CDN (FIX S10) -->
        ...
        <div class="mt-4 sm:mt-5 text-center space-y-2">
            <p class="text-gray-500 text-xs sm:text-sm">
                <a routerLink="/forgot-password" class="text-black font-semibold hover:underline transition-colors">Forgot your password?</a>
            </p>
            <p class="text-gray-500 text-xs sm:text-sm">
                Don't have an account? 
                <a routerLink="/signup" class="text-black font-bold hover:underline transition-colors">Sign up free</a>
            </p>
        </div>
      </div>
    </div>
...
export class LoginComponent {
  ...
  // FIX: Method wrapper needed because arrow fns with => break Angular template parser
