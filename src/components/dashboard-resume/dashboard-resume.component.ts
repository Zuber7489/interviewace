...
@Component({
  ...
  template: `
    <div class="max-w-4xl mx-auto">
      <h1 class="text-3xl font-bold text-black mb-8">Upload Resume</h1>
      
      <div class="glass-card p-8 rounded-2xl border border-black/5">
        <div class="mb-8">
          <h2 class="text-xl font-bold text-black mb-4">Upload PDF Resume</h2>
          <label
            class="group block w-full h-64 border-2 border-dashed border-gray-300 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-black/50 hover:bg-black/5 transition-all duration-300">
            <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
              <i class="fas fa-cloud-upload-alt text-3xl text-black"></i>
            </div>
            <span class="text-lg font-medium text-gray-600 group-hover:text-black transition-colors">
              Drag & drop your resume here
            </span>
            <span class="text-sm text-gray-500 mt-2">or click to browse</span>
            <span class="text-xs text-gray-400 mt-4">Maximum size: 5MB</span>
            <input type="file" class="hidden" accept=".pdf" (change)="onFileSelected($event)">
          </label>
        </div>

        @if(resumeFileName()) {
        <div class="flex items-center justify-between bg-black/10 border border-black/20 p-4 rounded-xl mb-8">
          <div class="flex items-center gap-3">
            <i class="fas fa-file-pdf text-2xl text-black"></i>
            <div>
              <p class="font-medium text-black">{{ resumeFileName() }}</p>
              <p class="text-xs text-gray-600">{{ fileSize() }}</p>
            </div>
          </div>
          <button (click)="removeFile()" class="text-red-600 hover:text-red-800 transition-colors">
        ...
        <div class="border-t border-black/10 pt-8">
          <h2 class="text-xl font-bold text-black mb-6">Or Enter Details Manually</h2>
          ...
          <form (submit)="onSubmit()" class="space-y-6">
            ...
            <button type="submit"
              class="w-full bg-black hover:bg-gray-800 text-white font-bold py-4 rounded-xl transition-all shadow-lg hover:shadow-xl">
              Save Profile
            </button>
          </form>
        </div>
